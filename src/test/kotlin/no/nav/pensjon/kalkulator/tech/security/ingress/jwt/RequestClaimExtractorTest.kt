...
class RequestClaimExtractorTest {

    @Mock
    private lateinit var request: HttpServletRequest

    @Test
    fun `extractAuthorizationClaim extracts claim from Authorization header JWT`() {
        `when`(request.getHeader("Authorization")).thenReturn(JWT)
        val claim = RequestClaimExtractor().extractAuthorizationClaim(request, "idtyp")
        assertEquals("app", claim)
    }
